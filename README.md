...
You can find the required permissions in the [Requirements section](https://docs.mender.io/overview/requirements#amazon-s3-iam-policies) of the official documentation.
...
export AWS_BUCKET="replace-with-your-bucket-name"
```
...
```bash
export MENDER_SERVER_DOMAIN="mender.example.org"
...
ingress:
  enabled: true
  annotations:
    <your-annotations depending your ingress controller>
  hosts:
...
## Upgrading from Helm Chart 5.x

With the latest changes in the Helm Chart for the Mender Server,
starting from version 3.8 of the Mender Server, we’ve made some important
...
The Helm Chart v.6.x includes the following external services, enabled
by default, to ease the
installation process of the Mender Server Open Source:

* MongoDB
* SeaweedFS
* NATS

Redis could be enabled, but it's used only for the Mender Server Enterprise.

**Warning**: for production usage, it's recommended to use external services
instead of the ones included in the Helm Chart.

Until version 6.x, the external services were disabled by default, so if
you are using your own service, please make sure to disable them explicitly:
```yaml
redis:
  enabled: false
...
Starting from version 6.x, the container images are managed by the `default`
section. Additionally, the tag drop the `mender-` prefix, so you should:
...
default:
  image:
    ...
    tag: 3.8.0
```
The default values is handling the repository and the tag, so you should
simply unset both the `global.image` and the `default.image` sections:
...
### Registry credentials (Enterprise Only)
Starting from version 6.x, the registry credentials are removed from the
values file, for security reason. You have to create the secret manually
before installing the new version of the Helm Chart.

```bash
export MENDER_REGISTRY_USERNAME="<your_user>"
export MENDER_REGISTRY_PASSWORD="<your_password>"
export MENDER_REGISTRY_EMAIL="<your_email>"
...
Then you can reference the created secret in the values.yaml file, and comment out or remove the `global.image:`:
